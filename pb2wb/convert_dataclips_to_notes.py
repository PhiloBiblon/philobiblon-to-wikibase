...
# Parse the replacement argument
...
def post_notes(q_number, text):
    ...
    processed_qnums.append(q_number)  # Keep track of processed QNUMs
...
def join_qnum_to_pbid(qnumbers):
    processed_df = df[df[second_column].isin(qnumbers)]
    # Select only QNUMBER and PBID columns
    output_df = processed_df[[second_column, first_column]].rename(columns={second_column: 'QNUMBER', first_column: 'PBID'})
    #Remove duplicates
    output_df = output_df.drop_duplicates(subset=['QNUMBER', 'PBID'])
    output_df.to_csv(f"processed_{bibliography}_{instance}_{table}_notes.csv", index=False)
    print(f"Processed QNUMs saved csv")
    print(f'Failed QNUMs: {failed_qnums}')
    ...
    with open('grouped_texts.txt', 'w') as file:
        for group_key, text in group_texts.items():
            if not dry_run:
                ...
                #file.write(f"{text}\n") #just for testing
    print(f"Total notes processed: {count}")
    print('Creating csv file of processed QNUMs and PBIDs')
    join_qnum_to_pbid(processed_qnums)
