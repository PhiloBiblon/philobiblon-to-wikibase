...
df = pd.read_csv('BITAGAP_geo_checklist.csv')
df2 = pd.read_csv('BITAGAP_rec_checklist.csv')
df3 = pd.read_csv('fg_places_300k.csv')

# Remove rows from df1 that match id's in df2
#df2 = df2[~df2['FG ID'].isin(df['GEOID'])]
df2 = df[~df['GEOID'].isin(df2['Apt'])]
df2.to_csv('bitagap_geo_reconciled.csv', index=False)
