...
def preprocess(table):
  ...
  if table is None or table is Table.BIBLIOGRAPHY:
    GenericPreprocessor().preprocess(get_full_input_path('BETA/csvs/beta_bibliography.csv'), beta_pre_processed_dir)
  if table is None or table is Table.BIOGRAPHY:
    BiographyPreprocessor().preprocess(get_full_input_path('BETA/csvs/beta_biography.csv'), get_full_input_path('BETA/csvs/beta_geography.csv'), beta_pre_processed_dir)
  ...
  if table is None or table is Table.LIBRARY:
    GenericPreprocessor().preprocess(get_full_input_path('BETA/csvs/beta_library.csv'), beta_pre_processed_dir)
  if table is None or table is Table.MS_ED:
    GenericPreprocessor().preprocess(get_full_input_path('BETA/csvs/beta_ms_ed.csv'), beta_pre_processed_dir)
  if table is None or table is Table.SUBJECT:
    GenericPreprocessor().preprocess(get_full_input_path('BETA/csvs/beta_subject.csv'), beta_pre_processed_dir)
  if table is None or table is Table.UNIFORM_TITLE:
    GenericPreprocessor().preprocess(get_full_input_path('BETA/csvs/beta_uniform_title.csv'), beta_pre_processed_dir)
