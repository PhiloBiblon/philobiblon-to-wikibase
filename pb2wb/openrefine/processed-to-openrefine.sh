#!/bin/bash

today=`date +"%Y-%m-%d"`

## Gather alternate project tag from command line
while getopts "t": flag
do
    case "${flag}" in
        t) version=${OPTARG};;
    esac
done

if [[ -z $version ]]; then
  echo "Alternate project tag not supplied, using default date: ${today}"
  tag=${today}
fi  

## Identify location of processed files on local machine
echo "Looking for processed files on local machine"
if processed_path=`find ~ -type d -name "philobiblon-private-master" 2>&1 | grep -v "Operation not permitted"`; then
  echo "Using processed file path: ${processed_path}"
else
  echo "Unable to find processed files, please verify files exist"
  exit 1
fi

echo "Proceeding using the tag: ${tag}"

## Loop through processed files and create openrefine projects with updated version number
for f in ${processed_path}/data/processed/pre/BETA/*
do
  echo $f && openrefine-client -H philobiblon.cog.berkeley.edu -P 3333 --projectName=`basename $f | sed 's/.csv/.'${tag}'/'` --create $f
done

