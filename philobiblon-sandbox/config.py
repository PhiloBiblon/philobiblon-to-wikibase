...
wikibase_main_page = 'http://philobiblon.duckdns.org/wiki/Main_Page'
...
import re
q_re = re.compile(r'(<?https?://philobiblon.duckdns.org/(entity|wiki)/)?(Q[0-9]+)>?')
q_re_group_id = 3
p_re = re.compile(r'(<?https?://philobiblon.duckdns.org/(entity/|wiki/Property:))?(P[0-9]+)>?')
p_re_group_id = 3
...
identifier_space = 'http://philobiblon.duckdns.org/entity/'
schema_space = 'http://philobiblon.duckdns.org/prop/direct/'
...
qid_url_pattern = 'http://philobiblon.duckdns.org/wiki/{{id}}'
...
service_name = 'Philobiblon Wikibase'

# URL (without the trailing slash) where this server runs
this_host = 'http://philobiblon.duckdns.org:8000'
...
SELECT ?pid WHERE { ?pid wdt:P1/wdt:P2* wd:Q2 }
"""
